...
public class FinanceRepresentativePage extends TestBase{
	...
	public FinanceRepresentativePage(WebDriver driver) {
		driver1 = driver ;
	...
	public boolean fillProjectCreation(String datapoolPath,String sheetName,String projectState,int tcRowNum) throws Exception{
...
//				}
				...
				try
				{
					  isProjectCreated = isAlertMessagePresent();
				}
				catch(Exception e) 
				{
					logError("Fail to Save the Internal Project -> "+ e.getMessage());
				}
				threadsleep(2000);
			}
			else
...
//						{
//							logInfo("Is Alert Displayed?: " + isAlertDisplayed);
//							logInfo("Retry Alert Count is: " +count);
//							alertMsg= Alert.getText().trim();	
//							count = count+1;
//							threadsleep(1000);
//							if(alertMsg.contains("Request Approved"))
//							{
//								alertMsg= Alert.getText().trim();	
//								logInfo("Alert Message is Displayed with Message: " +alertMsg);
...
//						}
		...
		threadsleep(2000);
		logInfo("Is Project Created Successfully? " + isProjectCreated);
		return  isProjectCreated;
		
  } // End of fillProjectCreation()
	
	
// Function to Check Alert Message is Present after Project Saved
	public boolean isAlertMessagePresent() {
		...
		try
		{
			while(!isAlertDisplayed)
			{
				...
				if(alertMsg.contains("Request Approved"))
				{
					...
					//op.clickElement(NewProjectLink);
					isAlertDisplayed = true;
					logInfo("Success Alert Message Flag is set to? " + isAlertDisplayed);
					isProjectSaved = true;
				}
			    if(alertMsg.contains("Due to technical error"))
				{
					alertMsg= Alert.getText().trim();	
					logInfo("Alert Message is Displayed with Message: " +alertMsg);
					logInfo("Error While Saving the Project");
					//op.clickElement(NewProjectLink);
					isAlertDisplayed = true;
					logInfo("Error Alert Message Flag is set to? " + isAlertDisplayed);
		...
		{
			logInfo("Unable to locate element : " + e.getMessage()) ;
		}
	...
	try{
			...
			logInfo("New Project Link is Visible");
			op.clickElement(NewProjectLink);
			wait.until(ExpectedConditions.visibilityOf(CreateProjectLink));
			logInfo("Create New Project Link is Visible");
